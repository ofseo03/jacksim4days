...
앱은 **로그인 이후에 사용**하는 구조입니다: 랜딩 → `/login` → 온보딩 →
대시보드. 대시보드·습관·통계·프로필·설정·온보딩은 로그인 게이트
(`AuthGate`)로 보호되며, 미로그인 상태로 접근하면 `/login`으로 이동합니다.
프로필 페이지의 **계정** 카드에서 계정 확인/로그아웃이 가능합니다.

부팅 시 만들어지는 익명 세션은 뒤에서 유지되다가 로그인 순간 계정에
연결되고(`linkIdentity`), 다른 계정으로 전환하면 그 계정의 데이터만
보입니다 — 데이터는 계정(`user_id`)별로 RLS로 격리됩니다.
Supabase 환경 변수가 없으면 로그인이 불가능하므로 게이트가 꺼지고
**로컬 전용 모드**로 동작합니다.
...
| `/login` | 소셜 로그인 (구글·카카오·네이버·애플) — 앱 사용의 관문 |
